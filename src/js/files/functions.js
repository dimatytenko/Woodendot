...
export function fullVHfix() {
  const fullScreens = document.querySelectorAll(
    "[data-fullscreen]"
  );
...
export function spollers() {
  const spollersArray = document.querySelectorAll(
    "[data-spollers]"
  );
  if (spollersArray.length > 0) {
    // Получение обычных слойлеров
    const spollersRegular = Array.from(
      spollersArray
    ...
    let mdQueriesArray = dataMediaQueries(
      spollersArray,
      "spollers"
    );
    ...
    function initSpollers(
      ...
      spollersArray.forEach((spollersBlock) => {
        ...
        if (matchMedia.matches || !matchMedia) {
          ...
          spollersBlock.addEventListener(
            "click",
            setSpollerAction
          );
        } else {
      ...
      if (spollerTitles.length) {
        ...
        spollerTitles.forEach((spollerTitle) => {
          if (hideSpollerBody) {
            ...
            if (
              !spollerTitle.classList.contains(
                "_spoller-active"
              )
            ) {
    ...
    function setSpollerAction(e) {
      ...
      if (el.closest("[data-spoller]")) {
        ...
        const spollerSpeed = spollersBlock.dataset
          .spollersSpeed
    ...
    function hideSpollersBody(spollersBlock) {
      ...
      if (
        ...
        spollerActiveTitle.classList.remove(
          "_spoller-active"
        );
        _slideUp(
...
export function tabs() {
  ...
  function setTitlePosition(tabsMediaArray, matchMedia) {
    tabsMediaArray.forEach((tabsMediaItem) => {
      ...
      let tabsTitles = tabsMediaItem.querySelector(
        "[data-tabs-titles]"
      );
      let tabsTitleItems = tabsMediaItem.querySelectorAll(
        "[data-tabs-title]"
      );
      let tabsContent = tabsMediaItem.querySelector(
        "[data-tabs-body]"
      );
      ...
      tabsTitleItems = Array.from(tabsTitleItems).filter(
        (item) =>
          item.closest("[data-tabs]") === tabsMediaItem
      );
      tabsContentItems = Array.from(
        tabsContentItems
      ).filter(
        (item) =>
          item.closest("[data-tabs]") === tabsMediaItem
  ...
  function initTabs(tabsBlock) {
    let tabsTitles = tabsBlock.querySelectorAll(
      "[data-tabs-titles]>*"
    );
    let tabsContent = tabsBlock.querySelectorAll(
      "[data-tabs-body]>*"
    );
    const tabsBlockIndex = tabsBlock.dataset.tabsIndex;
    const tabsActiveHashBlock =
      tabsActiveHash[0] == tabsBlockIndex;
    ...
    if (tabsContent.length) {
      ...
      tabsContent.forEach((tabsContentItem, index) => {
        ...
        if (
          tabsActiveHashBlock &&
          index == tabsActiveHash[1]
        ) {
          tabsTitles[index].classList.add("_tab-active");
        }
        tabsContentItem.hidden =
          !tabsTitles[index].classList.contains(
            "_tab-active"
          );
      });
    }
  ...
  function setTabsStatus(tabsBlock) {
    ...
    if (tabsContent.length > 0) {
      const isHash = tabsBlock.hasAttribute(
        "data-tabs-hash"
      );
      tabsContent = Array.from(tabsContent).filter(
        (item) => item.closest("[data-tabs]") === tabsBlock
    ...
    if (el.closest("[data-tabs-title]")) {
      ...
      if (
        ...
        tabActiveTitle.length
          ...
          : null;
        tabActiveTitle.length
          ? tabActiveTitle[0].classList.remove(
              "_tab-active"
            )
          : null;
...
export function showMore() {
  window.addEventListener("load", function (e) {
    const showMoreBlocks = document.querySelectorAll(
      "[data-showmore]"
    );
    ...
    if (showMoreBlocks.length) {
      ...
      showMoreBlocksRegular = Array.from(
        showMoreBlocks
      ).filter(function (item, index, self) {
      ...
      mdQueriesArray = dataMediaQueries(
        showMoreBlocks,
        "showmoreMedia"
      );
      if (mdQueriesArray && mdQueriesArray.length) {
    ...
    function initItem(showMoreBlock, matchMedia = false) {
      ...
      showMoreContent = Array.from(showMoreContent).filter(
        (item) =>
          item.closest("[data-showmore]") === showMoreBlock
      )[0];
      ...
      if (matchMedia.matches || !matchMedia) {
        if (
          hiddenHeight < getOriginalHeight(showMoreContent)
        ) {
    ...
    function showMoreActions(e) {
      ...
      if (targetType === "click") {
        if (targetEvent.closest("[data-showmore-button]")) {
          ...
          const showMoreBlock = showMoreButton.closest(
            "[data-showmore]"
          );
          const showMoreContent =
            showMoreBlock.querySelector(
              "[data-showmore-content]"
            );
          const showMoreSpeed = showMoreBlock.dataset
            .showmoreButton
          ...
          if (
            !showMoreContent.classList.contains("_slide")
          ) {
            showMoreBlock.classList.contains(
              "_showmore-active"
            )
              ? _slideUp(
                  showMoreContent,
            ...
            showMoreBlock.classList.toggle(
              "_showmore-active"
            );
          }
        }
...
export function dataMediaQueries(array, dataSetValue) {
  // Получение объектов с медиа запросами
  const media = Array.from(array).filter(function (
    item,
    index,
    self
  ) {
